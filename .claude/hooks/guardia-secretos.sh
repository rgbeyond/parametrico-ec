...
# FALLA CERRADO.
#
# La version anterior terminaba cada extraccion con `2>/dev/null || echo ""`.
# Eso convierte cualquier fallo -JSON malformado, un campo renombrado en una
# version futura de Claude Code, python3 ausente- en "ruta vacia, cuerpo
# vacio", y el guardia aprueba. Es el mismo modo de fallo que ya nos mordio en
# la Fase 0, cuando el guardia destructivo quedo inerte y parecia funcionar.
#
# Un control que no puede leer su entrada NO SABE si la escritura es peligrosa.
# Aprobar en la duda es la via de escape silenciosa.
entrada=$(cat)

# stdin vacio no es una entrada rota: es que no hay evento. Ahi no hay nada que
...
if not isinstance(d, dict): raise ValueError('la entrada no es un objeto')
ti = d.get('tool_input')
if ti is None:
    print('__SIN_TOOL_INPUT__'); raise SystemExit(0)
if not isinstance(ti, dict): raise ValueError('tool_input no es un objeto')
ruta = ti.get('file_path') or ''
# El cuerpo puede venir en varias claves segun la herramienta. Se concatenan
# TODAS las de texto: si manana aparece otra, entra sola en vez de quedar
# fuera del analisis en silencio.
partes = []
for k in ('content', 'new_string', 'newString'):
    v = ti.get(k)
...
ruta=$(printf '%s' "$campos" | head -1)
cuerpo=$(printf '%s' "$campos" | tail -n +2)
